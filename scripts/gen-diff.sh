...
# The base generator gets its own target dir. It cannot share the working
# tree's: base and head are the same `openapi-to-rust` package, and a shared
# dir lets a restored build cache make the head crate look fresh after the base
# build has overwritten the shared artifact. Cargo then skips relinking and the
# "head" corpus is generated by the base binary, which reports "no change"
# while the manifest check sees stale output. A separate dir makes the two
# sides' artifacts independent. It lives under `target/` so rust-cache keeps it
# warm across runs.
BASE_TARGET="${GEN_DIFF_BASE_TARGET_DIR:-$PWD/target/gen-diff-base}"
...
if [ "$CACHE_OK" = "1" ]; then
  ...
  # The base side builds into its own target dir (see BASE_TARGET above), so
  # its artifact can never be confused with the head build's. Dependencies are
  # compiled once per dir; the dir persists under target/ for rust-cache.
  BASE_BIN="$(corpus_build "$PWD/$WT" "$BASE_TARGET" "$PROFILE")"
